...
async function verifyToken(req) {
  ...
  if (!token) {
    ...
    if (cookieHeader) {
      // Supposons que le cookie se nomme "token"
...
// Gestion des endpoints Spotify sécurisés pour admin
const spotifyHandlers = {
  async spotifyScrape(sendEvent) {
    // Vérification admin
  ...
  async spotifySyncIndividual(sendEvent, playlistId) {
    // Vérification admin
    await verifyAdmin(req);
...
const routeMap = {
  ...
  "GET:/api/live/spotify/scrape": () => new Response(createSSEStream(spotifyHandlers.spotifyScrape), {
    headers: { ...corsHeaders, "Content-Type": "text/event-stream", "Cache-Control": "no-cache", Connection: "keep-alive" }
  }),
  "GET:/api/live/spotify/sync": () => new Response(createSSEStream(spotifyHandlers.spotifySync), {
    headers: { ...corsHeaders, "Content-Type": "text/event-stream", "Cache-Control": "no-cache", Connection: "keep-alive" }
  }),
};
...
if (routeMap[routeKey]) {
  ...
  return new Response(createSSEStream((sendEvent) => spotifyHandlers.spotifySyncIndividual(sendEvent, playlistId)), {
    headers: { ...corsHeaders, "Content-Type": "text/event-stream", "Cache-Control": "no-cache", Connection: "keep-alive" }
  });
}
